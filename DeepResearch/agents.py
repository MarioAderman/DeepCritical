...
class BaseAgent(ABC):
    ...
    def _initialize_agent(self, system_prompt: Optional[str], instructions: Optional[str]):
    ...
    async def execute(self, input_data: Any, deps: Optional[AgentDependencies] = None) -> AgentResult:
    ...
    def execute_sync(self, input_data: Any, deps: Optional[AgentDependencies] = None) -> AgentResult:
...
- domain: Research domain (bioinformatics, general, etc.)"""
    ...
    def parse(self, question: str) -> Dict[str, Any]:
        ...
        return result.data if result.success else {"intent": "research", "query": question}
...
- success_criteria: How to measure success"""
    ...
    async def create_plan(self, parsed_question: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def _get_default_plan(self, query: str) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            {"tool": "evaluator", "params": {"question": query, "answer": "${finalize.final}"}},
        ]
...
class ExecutorAgent(BaseAgent):
    ...
    def __init__(self, model_name: str = "anthropic:claude-sonnet-4-0", retries: int = 2, **kwargs):
...
4. Collecting results"""
    ...
    async def execute_plan(self, plan: List[Dict[str, Any]], history: ExecutionHistory) -> Dict[str, Any]:
    ...
    def _materialize_params(self, params: Dict[str, Any], bag: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _adjust_parameters(self, params: Dict[str, Any], bag: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def run_plan(self, plan: List[Dict[str, Any]], history: ExecutionHistory) -> Dict[str, Any]:
...
- confidence: Confidence in results"""
    ...
    async def search(self, query: str, search_type: str = "search", num_results: int = 10) -> Dict[str, Any]:
        """Perform web search."""
        search_params = {
            "query": query,
...
- cross_references: Cross-references found"""
    ...
    def _register_tools(self):
        ...
        try:
            from .tools.bioinformatics_tools import (
                BioinformaticsFusionTool, BioinformaticsReasoningTool,
                BioinformaticsWorkflowTool, GOAnnotationTool, PubMedRetrievalTool
            )
    ...
    async def perform_reasoning(self, task: ReasoningTask, dataset: FusedDataset) -> Dict[str, Any]:
    ...
    def _register_tools(self):
        ...
        try:
            from .tools.deepsearch_tools import (
                WebSearchTool, URLVisitTool, ReflectionTool, 
                AnswerGeneratorTool, QueryRewriterTool
            )
    ...
    def _initialize_deep_agent(self):
        ...
        try:
            config = AgentConfig(
                ...
                capabilities=[AgentCapability.PLANNING, AgentCapability.TASK_MANAGEMENT],
                max_iterations=5,
                timeout=120.0
            )
    ...
    def _initialize_deep_agent(self):
        ...
        try:
            config = AgentConfig(
                ...
                capabilities=[AgentCapability.FILESYSTEM, AgentCapability.CONTENT_MANAGEMENT],
                max_iterations=3,
                timeout=60.0
            )
...
- recommendations: File organization recommendations"""
    ...
    def _register_tools(self):
        ...
        try:
            from .tools.deep_agent_tools import list_files_tool, read_file_tool, write_file_tool, edit_file_tool
...
- performance_metrics: Performance metrics"""
    ...
    async def execute_parallel_tasks(self, tasks: List[Dict[str, Any]], context: Optional[DeepAgentState] = None) -> List[AgentExecutionResult]:
    ...
    def _initialize_deep_agent(self):
        ...
        try:
            config = AgentConfig(
                ...
                capabilities=[AgentCapability.ORCHESTRATION, AgentCapability.TASK_DELEGATION, AgentCapability.RESEARCH],
                max_iterations=20,
                timeout=900.0
            )
...
- recommendations: Recommendations for future tasks"""
    ...
    def _register_tools(self):
        ...
        try:
            from .tools.deep_agent_tools import task_tool, write_todos_tool, list_files_tool, read_file_tool
            from .tools.websearch_tools import WebSearchTool
    ...
    async def handle_general_task(self, task_description: str, context: Optional[DeepAgentState] = None) -> AgentExecutionResult:
...
class MultiAgentOrchestrator:
    ...
    def _initialize_agents(self):
        ...
        if self.config.get("deep_agent", {}).get("enabled", False):
            self.agents[AgentType.DEEP_AGENT_PLANNING] = DeepAgentPlanningAgent(model_name)
            self.agents[AgentType.DEEP_AGENT_FILESYSTEM] = DeepAgentFilesystemAgent(model_name)
            self.agents[AgentType.DEEP_AGENT_RESEARCH] = DeepAgentResearchAgent(model_name)
            self.agents[AgentType.DEEP_AGENT_ORCHESTRATION] = DeepAgentOrchestrationAgent(model_name)
            self.agents[AgentType.DEEP_AGENT_GENERAL] = DeepAgentGeneralAgent(model_name)
    
    async def execute_workflow(self, question: str, workflow_type: str = "research") -> Dict[str, Any]:
        ...
        try:
            ...
            if workflow_type == "bioinformatics":
                result = await self._execute_bioinformatics_workflow(question, parsed, plan)
    ...
    async def _execute_standard_workflow(self, question: str, parsed: Dict[str, Any], plan: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def _execute_bioinformatics_workflow(self, question: str, parsed: Dict[str, Any], plan: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def _execute_deepsearch_workflow(self, question: str, parsed: Dict[str, Any], plan: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def _execute_rag_workflow(self, question: str, parsed: Dict[str, Any], plan: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def _execute_deep_agent_workflow(self, question: str, parsed: Dict[str, Any], plan: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if AgentType.DEEP_AGENT_ORCHESTRATION in self.agents:
            ...
            result = await orchestration_agent.orchestrate_tasks(question, initial_state)
            
            if result.success:
                return {
                    "deep_agent_result": result.result,
                    "answer": result.result.get("result_synthesis", "DeepAgent orchestration completed"),
                    "execution_metadata": {
                        "execution_time": result.execution_time,
                        "tools_used": result.tools_used,
                        "iterations_used": result.iterations_used
...
__all__ = [
    "BaseAgent", "ParserAgent", "PlannerAgent", "ExecutorAgent",
    "SearchAgent", "RAGAgent", "BioinformaticsAgent", "DeepSearchAgent",
    "EvaluatorAgent", "MultiAgentOrchestrator",
    # DeepAgent classes
    "DeepAgentPlanningAgent", "DeepAgentFilesystemAgent", "DeepAgentResearchAgent",
    "DeepAgentOrchestrationAgent", "DeepAgentGeneralAgent",
    "AgentType", "AgentStatus", "AgentDependencies", "AgentResult",
    "ExecutionHistory", "create_agent", "create_orchestrator"
]
