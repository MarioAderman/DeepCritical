...
class GOAnnotation(BaseModel):
    ...
    full_text: Optional[str] = Field(None, description="Full text for open access papers")
...
class GeneExpressionProfile(BaseModel):
    ...
    expression_values: Dict[str, float] = Field(..., description="Expression values by sample ID")
    ...
    adjusted_p_value: Optional[float] = Field(None, description="Adjusted p-value (FDR)")
...
class DrugTarget(BaseModel):
    ...
    action: Optional[str] = Field(None, description="Drug action (inhibitor, activator, etc.)")
    ...
    clinical_phase: Optional[str] = Field(None, description="Clinical development phase")
...
class PerturbationProfile(BaseModel):
    ...
    gene_expression_changes: Dict[str, float] = Field(..., description="Gene expression changes")
...
class ProteinStructure(BaseModel):
    ...
    binding_sites: List[Dict[str, Any]] = Field(default_factory=list, description="Binding sites")
    publication_date: Optional[datetime] = Field(None, description="Publication date")
...
class FusedDataset(BaseModel):
    ...
    go_annotations: List[GOAnnotation] = Field(default_factory=list, description="GO annotations")
    pubmed_papers: List[PubMedPaper] = Field(default_factory=list, description="PubMed papers")
    geo_series: List[GEOSeries] = Field(default_factory=list, description="GEO series")
    gene_expression_profiles: List[GeneExpressionProfile] = Field(default_factory=list, description="Gene expression profiles")
    drug_targets: List[DrugTarget] = Field(default_factory=list, description="Drug targets")
    perturbation_profiles: List[PerturbationProfile] = Field(default_factory=list, description="Perturbation profiles")
    protein_structures: List[ProteinStructure] = Field(default_factory=list, description="Protein structures")
    protein_interactions: List[ProteinInteraction] = Field(default_factory=list, description="Protein interactions")
    
    # Metadata
    total_entities: int = Field(0, description="Total number of entities")
    cross_references: Dict[str, List[str]] = Field(default_factory=dict, description="Cross-references between entities")
    ...
    def calculate_total_entities(cls, v, values):
        ...
        total = 0
        for field_name in ['go_annotations', 'pubmed_papers', 'geo_series', 
                          'gene_expression_profiles', 'drug_targets', 
                          'perturbation_profiles', 'protein_structures', 
...
class ReasoningTask(BaseModel):
    ...
    required_evidence: List[EvidenceCode] = Field(default_factory=list, description="Required evidence codes")
    supporting_data: List[str] = Field(default_factory=list, description="Supporting data identifiers")
...
class DataFusionRequest(BaseModel):
    ...
    fusion_type: str = Field(..., description="Type of fusion (GO+PubMed, GEO+CMAP, etc.)")
    source_databases: List[str] = Field(..., description="Source databases to fuse")
    filters: Dict[str, Any] = Field(default_factory=dict, description="Filtering criteria")
