...
class DataLoaderConfig(BaseModel):
    ...
    parameters: Dict[str, Any] = Field(default_factory=dict, description="Loader parameters")
...
class WorkflowExecution(BaseModel):
    ...
    child_execution_ids: List[str] = Field(default_factory=list, description="Child execution IDs")
...
class MultiAgentSystemConfig(BaseModel):
    ...
    coordination_strategy: str = Field("sequential", description="Coordination strategy")
...
class WorkflowOrchestrationConfig(BaseModel):
    ...
    primary_workflow: WorkflowConfig = Field(..., description="Primary REACT workflow config")
    sub_workflows: List[WorkflowConfig] = Field(default_factory=list, description="Sub-workflow configs")
    data_loaders: List[DataLoaderConfig] = Field(default_factory=list, description="Data loader configs")
    multi_agent_systems: List[MultiAgentSystemConfig] = Field(default_factory=list, description="Multi-agent system configs")
    judges: List[JudgeConfig] = Field(default_factory=list, description="Judge configs")
    execution_strategy: str = Field("parallel", description="Execution strategy (parallel, sequential, hybrid)")
    max_concurrent_workflows: int = Field(5, description="Maximum concurrent workflows")
    global_timeout: Optional[float] = Field(None, description="Global timeout in seconds")
...
class WorkflowResult(BaseModel):
    ...
    quality_score: Optional[float] = Field(None, description="Quality score from judges")
...
class HypothesisDataset(BaseModel):
    ...
    dataset_id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Dataset ID")
    ...
    source_workflows: List[str] = Field(default_factory=list, description="Source workflow names")
...
class HypothesisTestingEnvironment(BaseModel):
    ...
    environment_id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Environment ID")
...
class ReasoningResult(BaseModel):
    ...
    reasoning_id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Reasoning ID")
    ...
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Reasoning metadata")
...
class WorkflowComposition(BaseModel):
    ...
    composition_id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Composition ID")
    ...
    expected_outputs: Dict[str, str] = Field(default_factory=dict, description="Expected outputs by workflow")
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "selected_workflows": ["bioinformatics_workflow", "rag_workflow", "reasoning_workflow"],
                "execution_order": ["rag_workflow", "bioinformatics_workflow", "reasoning_workflow"]
            }
        }
...
class OrchestrationState(BaseModel):
    ...
    state_id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="State ID")
    active_executions: List[WorkflowExecution] = Field(default_factory=list, description="Active executions")
    completed_executions: List[WorkflowResult] = Field(default_factory=list, description="Completed executions")
    pending_workflows: List[WorkflowConfig] = Field(default_factory=list, description="Pending workflows")
    current_composition: Optional[WorkflowComposition] = Field(None, description="Current composition")
...
class BreakCondition(BaseModel):
    ...
    custom_function: Optional[str] = Field(None, description="Custom function for custom_loss type")
...
class NestedReactConfig(BaseModel):
    ...
    agent_roles: List[AgentRole] = Field(default_factory=list, description="Agent roles for this loop")
    tools: List[str] = Field(default_factory=list, description="Tools available to agents")
...
class AgentOrchestratorConfig(BaseModel):
    ...
    agent_role: AgentRole = Field(AgentRole.ORCHESTRATOR_AGENT, description="Role of the orchestrator agent")
    model_name: str = Field("anthropic:claude-sonnet-4-0", description="Model for the orchestrator")
    break_conditions: List[BreakCondition] = Field(default_factory=list, description="Break conditions")
    ...
    can_spawn_subgraphs: bool = Field(True, description="Whether this orchestrator can spawn subgraphs")
    can_spawn_agents: bool = Field(True, description="Whether this orchestrator can spawn agents")
...
class SubgraphConfig(BaseModel):
    ...
    state_machine_path: str = Field(..., description="Path to state machine implementation")
    ...
    tools: List[str] = Field(default_factory=list, description="Tools available in subgraph")
...
class AppConfiguration(BaseModel):
    ...
    primary_orchestrator: AgentOrchestratorConfig = Field(..., description="Primary orchestrator config")
    nested_react_configs: List[NestedReactConfig] = Field(default_factory=list, description="Nested REACT configurations")
    subgraph_configs: List[SubgraphConfig] = Field(default_factory=list, description="Subgraph configurations")
    loss_functions: List[BreakCondition] = Field(default_factory=list, description="Loss functions for end conditions")
    global_break_conditions: List[BreakCondition] = Field(default_factory=list, description="Global break conditions")
    execution_strategy: str = Field("adaptive", description="Overall execution strategy")
    max_total_iterations: int = Field(100, description="Maximum total iterations across all loops")
    max_total_time: float = Field(3600.0, description="Maximum total execution time in seconds")
