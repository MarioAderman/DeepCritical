    ...
    async def vectorize_documents(self, document_chunks: List[str]) -> List[List[float]]:
        ...
        for i in range(0, len(document_chunks), batch_size):
            ...
            try:
                ...
                raise RuntimeError(f"Failed to generate embeddings for batch {i//batch_size}: {e}")
...
class VLLMDeployment(BaseModel):
    ...
    embedding_config: Optional[VLLMEmbeddingServerConfig] = Field(None, description="Embedding server configuration")
    ...
    async def wait_for_servers(self) -> bool:
        ...
        if self.auto_start:
            ...
            embedding_ready = await self.start_embedding_server() if self.embedding_config else True
            ...
            while (not llm_ready or not embedding_ready) and retries < self.max_retries:
                ...
                embedding_ready = await self._check_server_health(
                    f"http://{self.embedding_config.host}:{self.embedding_config.port}/health"
                ) if self.embedding_config else True
                retries += 1
...
class VLLMRAGSystem(BaseModel):
    ...
    embeddings: Optional[VLLMEmbeddings] = Field(None, description="VLLM embeddings provider")
