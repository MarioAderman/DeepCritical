...
class Embedding:
    ...
    def to_query_param(self) -> str:
        ...
        if self.filters:
            filter_parts = [f"{f.column}.{f.operator.value}.{f.value}" for f in self.filters]
...
class InsertRequest:
    ...
    def to_json(self) -> Union[Dict[str, Any], List[Dict[str, Any]]]:
        ...
        if isinstance(self.data, list):
            if self.columns:
                return [{col: item.get(col) for col in self.columns} for item in self.data]
            return self.data
        else:
            if self.columns:
...
class PostgRESTClient:
    ...
    def get_table(self, table_name: str, schema_name: Optional[str] = None) -> Optional[Table]:
        """Get table by name."""
    ...
    def get_view(self, view_name: str, schema_name: Optional[str] = None) -> Optional[View]:
        """Get view by name."""
        schema_name = schema_name or self.config.schema
        schema = self.get_schema(schema_name)
        if schema:
    ...
    def get_function(self, function_name: str, schema_name: Optional[str] = None) -> Optional[Function]:
...
def create_order_by(column: str, direction: OrderDirection = OrderDirection.ASCENDING) -> OrderBy:
    """Create an order by clause."""
    return OrderBy(column=column, direction=direction)


def create_pagination(limit: Optional[int] = None, offset: Optional[int] = None) -> Pagination:
