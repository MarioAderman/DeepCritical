...
class Document(BaseModel):
    ...
    """
    id: str = Field(default_factory=lambda: generate_id("doc"), description="Unique document identifier")
    content: str = Field(..., description="Document content/text")
    chunks: List[Chunk] = Field(default_factory=list, description="Document chunks")
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Document metadata")
    embedding: Optional[Union[List[float], "np.ndarray"]] = Field(None, description="Document embedding vector")
    created_at: datetime = Field(default_factory=datetime.now, description="Creation timestamp")
    updated_at: Optional[datetime] = Field(None, description="Last update timestamp")
    
    # Bioinformatics-specific metadata fields
    bioinformatics_type: Optional[str] = Field(None, description="Type of bioinformatics data (GO, PubMed, GEO, etc.)")
    source_database: Optional[str] = Field(None, description="Source database identifier")
    cross_references: Dict[str, List[str]] = Field(default_factory=dict, description="Cross-references to other entities")
    quality_score: Optional[float] = Field(None, ge=0.0, le=1.0, description="Quality score for the document")
    ...
    def from_bioinformatics_data(cls, data: Any, **kwargs) -> "Document":
        ...
        if isinstance(data, GOAnnotation):
            ...
            metadata = {
                ...
                "submission_date": data.submission_date.isoformat() if data.submission_date else None
            }
        else:
...
class EmbeddingsConfig(BaseModel):
    ...
    num_dimensions: int = Field(1536, description="Number of dimensions in embedding vectors")
...
class RAGQuery(BaseModel):
    ...
    search_type: SearchType = Field(SearchType.SIMILARITY, description="Type of search to perform")
    ...
    retrieval_query: Optional[str] = Field(None, description="Custom retrieval query for advanced stores")
...
class RAGResponse(BaseModel):
    ...
    generated_answer: Optional[str] = Field(None, description="Generated answer from LLM")
...
class BioinformaticsRAGSystem(RAGSystem):
    ...
    async def query_bioinformatics(self, query: BioinformaticsRAGQuery) -> BioinformaticsRAGResponse:
        ...
        if query.bioinformatics_types:
            enhanced_filters["bioinformatics_type"] = {"$in": query.bioinformatics_types}
        ...
        for result in search_results:
            ...
            if doc.bioinformatics_type:
                bioinformatics_summary["bioinformatics_types"].add(doc.bioinformatics_type)
...
Answer:"""
        ...
        quality_metrics = {
            "average_score": sum(r.score for r in search_results) / len(search_results) if search_results else 0.0,
    ...
    async def fuse_bioinformatics_data(self, data_sources: Dict[str, List[Any]]) -> List[Document]:
    ...
    def _add_cross_references(self, documents: List[Document]) -> None:
        ...
        for doc in documents:
            ...
            if "gene_symbol" in metadata:
                gene_symbol = metadata["gene_symbol"]
                related_docs = [doc_id for doc_id in gene_groups[gene_symbol] if doc_id != doc.id]
                if related_docs:
                    cross_refs["related_gene_docs"] = related_docs
            
            if "pmid" in metadata:
                pmid = metadata["pmid"]
                related_docs = [doc_id for doc_id in pmid_groups[pmid] if doc_id != doc.id]
                if related_docs:
...
class BioinformaticsRAGQuery(BaseModel):
    ...
    bioinformatics_types: Optional[List[str]] = Field(None, description="Filter by bioinformatics data types")
    source_databases: Optional[List[str]] = Field(None, description="Filter by source databases")
    evidence_codes: Optional[List[str]] = Field(None, description="Filter by GO evidence codes")
    organisms: Optional[List[str]] = Field(None, description="Filter by organisms")
    gene_symbols: Optional[List[str]] = Field(None, description="Filter by gene symbols")
...
class BioinformaticsRAGResponse(BaseModel):
    ...
    bioinformatics_summary: Dict[str, Any] = Field(default_factory=dict, description="Summary of bioinformatics data")
    cross_references: Dict[str, List[str]] = Field(default_factory=dict, description="Cross-references found")
    quality_metrics: Dict[str, float] = Field(default_factory=dict, description="Quality metrics for retrieved data")
...
class RAGWorkflowState(BaseModel):
    ...
    bioinformatics_response: Optional[BioinformaticsRAGResponse] = Field(None, description="Bioinformatics RAG response")
    ...
    bioinformatics_data: Dict[str, Any] = Field(default_factory=dict, description="Bioinformatics data being processed")
    fusion_metadata: Dict[str, Any] = Field(default_factory=dict, description="Data fusion metadata")
