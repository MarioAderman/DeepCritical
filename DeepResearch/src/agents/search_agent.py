...
class SearchAgentConfig(BaseModel):
    ...
    enable_analytics: bool = Field(True, description="Whether to enable analytics tracking")
...
class SearchQuery(BaseModel):
    ...
    search_type: Optional[str] = Field(None, description="Type of search: 'search' or 'news'")
    ...
    async def get_analytics(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await self.agent.run(
                f"Get analytics data for the last {days} days",
                deps=context
            )
