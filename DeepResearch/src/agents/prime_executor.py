...
class ToolExecutor:
    ...
    def execute_workflow(self, context: ExecutionContext) -> Dict[str, Any]:
        ...
        for step_name in context.workflow.execution_order:
            ...
            if step_result.success:
                ...
                context.history.add_item(ExecutionItem(
                    step_name=step_name,
                    tool=step.tool,
    ...
    def _execute_step_with_retry(self, step: WorkflowStep, context: ExecutionContext) -> ExecutionResult:
                ...
                if context.manual_confirmation:
                    if not self._request_manual_confirmation(step, parameters):
                        return ExecutionResult(
                            success=False,
                            error="Manual confirmation denied",
                            data={}
                        )
        ...
        return ExecutionResult(
            success=False,
            error=f"Step failed after {self.retries} retries",
            data={}
        )
    
    def _validate_step_inputs(self, step: WorkflowStep, context: ExecutionContext) -> ExecutionResult:
    ...
    def _validate_step_outputs(self, step: WorkflowStep, result: ExecutionResult) -> ExecutionResult:
    ...
    def _prepare_parameters(self, step: WorkflowStep, context: ExecutionContext) -> Dict[str, Any]:
        """Prepare parameters with data substitution."""
    ...
    def _check_success_criteria(self, step: WorkflowStep, result: ExecutionResult) -> ExecutionResult:
    ...
    def _handle_failure_with_replanning(self, failed_step: WorkflowStep, context: ExecutionContext) -> Optional[Dict[str, Any]]:
        ...
        if alternative_tool:
            print(f"Strategic re-planning: substituting {failed_step.tool} with {alternative_tool}")
    ...
    def _adjust_parameters_tactically(self, step: WorkflowStep) -> Optional[Dict[str, Any]]:
        ...
        # Adjust exhaustiveness for docking
            ...
            adjusted["exhaustiveness"] = min(adjusted["exhaustiveness"] * 2, 32)  # More thorough
    ...
    def _should_continue_after_failure(self, step: WorkflowStep, context: ExecutionContext) -> bool:
        ...
        failed_steps = sum(1 for item in context.history.items if item.status == ExecutionStatus.FAILED)
...
def execute_workflow(workflow: WorkflowDAG, registry: ToolRegistry, 
                    manual_confirmation: bool = False, 
                    adaptive_replanning: bool = True) -> Dict[str, Any]:
