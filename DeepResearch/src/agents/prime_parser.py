...
class QueryParser:
    ...
    def _analyze_semantic_intent(self, query: str) -> ScientificIntent:
        ...
        if any(word in query_lower for word in ["design", "create", "generate", "novel"]):
        ...
        if any(word in query_lower for word in ["classify", "classification", "category"]):
    ...
    def _analyze_syntactic_formats(self, query: str) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    def _determine_domain(self, query: str) -> str:
        ...
        if any(word in query_lower for word in ["antibody", "immunoglobulin", "therapeutic"]):
            return "immunology"
    ...
    def _assess_complexity(self, query: str, intent: ScientificIntent) -> str:
        ...
        if intent in [ScientificIntent.DE_NOVO_DESIGN, ScientificIntent.MOLECULAR_DOCKING]:
            return "complex"
        elif intent in [ScientificIntent.PROTEIN_DESIGN, ScientificIntent.BINDING_ANALYSIS]:
            return "moderate"
        else:
            return "simple"
