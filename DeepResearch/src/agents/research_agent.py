...
from DeepResearch.tools.pyd_ai_tools import _build_builtin_tools, _build_toolsets, _build_agent as _build_core_agent
...
def _compose_agent_system(cfg: DictConfig, url_list: List[str] | None = None, bad_requests: List[str] | None = None, beast: bool = False) -> str:
	...
	sections: List[str] = [
		header.replace("${current_date_utc}", getattr(__import__("datetime").datetime.utcnow(), "strftime")("%a, %d %b %Y %H:%M:%S GMT"))
	]

	# Visit
	visit = loader.get("agent", "action_visit")
	if url_list:
		url_lines = "\n".join([f"  - [idx={i+1}] [weight=1.00] \"{u}\": \"...\"" for i, u in enumerate(url_list or [])])
	...
	if search:
		bad = ""
		if bad_requests:
			bad = "- Avoid those unsuccessful search requests and queries:\n<bad-requests>\n" + "\n".join(bad_requests) + "\n</bad-requests>"
	...
	# Wrapper + footer
	sections.append(actions_wrapper.replace("${action_sections}", "\n\n".join([s for s in sections[1:]])))
...
class ResearchAgent:
	...
	def run(self, question: str) -> ResearchOutcome:
		...
		if agent is None:
			return ResearchOutcome(answer="", references=[], context={"error": "pydantic_ai missing"})
		...
		for step in range(1, self.max_steps + 1):
			...
			refs = [r.get("url") if isinstance(r, dict) else str(r) for r in (maybe_refs or []) if r]
			break
		...
		return ResearchOutcome(answer=final_answer, references=refs, context={
			"visited": visited,
			"urls": url_pool,
