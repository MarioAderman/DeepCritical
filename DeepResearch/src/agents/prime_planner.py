...
class PlanGenerator:
    ...
    def _build_domain_heuristics(self) -> Dict[ScientificIntent, List[str]]:
        ...
        return {
            ScientificIntent.PROTEIN_DESIGN: [
                "uniprot_query", "alphafold2", "rfdiffusion", "evolla"
            ],
            ScientificIntent.BINDING_ANALYSIS: [
                "uniprot_query", "alphafold2", "autodock_vina", "diffdock"
            ],
            ScientificIntent.STRUCTURE_PREDICTION: [
                "uniprot_query", "alphafold2", "esmfold"
            ],
            ScientificIntent.FUNCTION_PREDICTION: [
                "uniprot_query", "hmmer_search", "evolla", "saprot"
            ],
            ScientificIntent.SEQUENCE_ANALYSIS: [
                "uniprot_query", "blast_search", "hmmer_search", "prot_trek"
            ],
            ScientificIntent.DE_NOVO_DESIGN: [
                "uniprot_query", "alphafold2", "rfdiffusion", "diffab"
            ],
            ScientificIntent.CLASSIFICATION: [
                "uniprot_query", "saprot", "evolla"
            ],
            ScientificIntent.REGRESSION: [
                "uniprot_query", "saprot", "evolla"
            ],
    ...
    def _generate_workflow_steps(self, problem: StructuredProblem, tools: List[str]) -> List[WorkflowStep]:
    ...
    def _generate_parameters(self, tool_spec: ToolSpec, problem: StructuredProblem) -> Dict[str, Any]:
    ...
    def _define_inputs(self, tool_spec: ToolSpec, problem: StructuredProblem, step_index: int) -> Dict[str, str]:
